...
表 \@ref(tab:tab-3) 中的数据已剔除价格因素影响，本例也可以运用excel进行数据处理。

（1）启动excel，建立数据文件“1990-2014年影响我国国内生产总值的各因素数据”。

（2）点击“数据”选项卡的“数据分析”选项（需提前激活“数据分析”工具包）；在弹出的对话框中，选择“回归”。

（3）页面显示回归对话框后，将“国内生产总值”的数据所在区域填入“Y值所在区域”，将“固定资产投资”、“就业人数”和
“社会消费品零售总额”的数据所在区域填入“X值所在区域”，设置置信度为95%，并选择相应的输出区域。

（4）点击“确定”按钮，得到“回归统计表”，“方差分析表”，“回归系数表”，结果显示与SPSS中的结果一致，“回归分析
表”即为SPSS中的表 \@ref(tab:tab-5)。

根据回归系数表可以得到回归模型，利用所得模型进行预测与上一节的步骤相同。
