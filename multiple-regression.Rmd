...
内生产总值、固定资产投资、就业人数以及社会消费品零售总额的数据如表 \@ref(tab:tab-4-1) 所示：
...
解：1.表 \@ref(tab:tab-4-1) 中的数据，国内生产总值、固定资产投资、社会消费品零售总额都是按现价核算的，需对数
据进行处理，因为它们都包含了价格变动因素的影响，因此需剔除价格影响后再建模估计参数。表 \@ref(tab:tab-5-1) 
已经对数据进行处理，即先依据《中国统计年鉴2015》中对应的各价格指数计算整理获得GDP价格定基指数（1990年为100）、
固定资产投资价格定基指数（1990年为100)和社会商品零售价格指数（1990年为100)；然后用现价数据除以相应的定基
价格指数，得到剔除价格影响之后的数据。

```{r tab-5-1, eval=T,results='markup', cache=F}
...
（1）启动excel，建立数据文件“1990-2014年影响我国国内生产总值的各因素数据”。
...
间为（`r tab16[2,6]` ， `r tab16[2,7]`）；就业人数的回归系数95%的置信区间为（`r tab16[3,6]` ， `r tab16[3,7]`）；
社会消费品零售总额的回归系数95%的置信区间为（`r tab16[4,6]` , `r tab16[4,7]`）。
...
```{r tab-17, eval=T,results='markup', cache=F}
...
表 \@ref(tab:tab-17) 给出了2015-2017年三个自变量数据和对应的GDP的预测值。

(2)由于多元回归的区间预测需要矩阵计算，用软件 R 进行矩阵计算更方便。根据公式
[@eq:model43] 和公式 [@eq:model49] 利用R软件可分别计算2015年不变价国内生产总值个
值和均值的区间预测为：
...
2017年GDP个值预测区间: $72675.07 \pm 2.08\times 7248.97$ =(`r x0[3,]%*%beta-qt(0.975,df=21)*sqrt(sigma^2*(1+t(x0[3,])%*%solve((t(x)%*%x))%*%x0[3,]))`，`r x0[3,]%*%beta+qt(0.975,df=21)*sqrt(sigma^2*(1+t(x0[3,])%*%solve((t(x)%*%x))%*%x0[3,])) `);

2017年GDP均值预测区间: $72675.07 \pm 2.08\times 6676.28$ =(`r x0[3,]%*%beta-qt(0.975,df=21)*sqrt(sigma^2*(t(x0[3,])%*%solve((t(x)%*%x))%*%x0[3,]))`，`r x0[3,]%*%beta+qt(0.975,df=21)*sqrt(sigma^2*(t(x0[3,])%*%solve((t(x)%*%x))%*%x0[3,])) `).
