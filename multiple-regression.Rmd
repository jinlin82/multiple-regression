...
{\mathop{\rm cov}} ({\varepsilon _i},{\varepsilon _j}) = \left\{ \begin{array}{l}
{\sigma ^2},\;\;i = j\\
0{\rm{ }},\;{\rm{ }}i \ne j
...
$$\hat y_0-t\times \hat \sigma \sqrt{1+x_0(X^TX)^{-1}x_0^T}<y_0<\hat y_0+t\times \hat \sigma \sqrt{1+x_0(X^TX)^{-1}x_0^T}$$ {#eq:model43}
...
要求：以国内生产总值(y)为因变量，固定资产投资(x1)、就业人数(x2)和社会消费品零售总额(x3)为自变量，拟合
影响国内生产总值的多元回归方程，并对回归方程进行检验，并解释模型的经济意义。
