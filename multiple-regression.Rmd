...
$$Y = X{\bf {\beta}}  + {\bf{\varepsilon}} $$ {#eq:model3}
...
对于回归方程式 [@eq:model6] 的图形，已不像一元线性回归时那样是一条直线，当 $p=2$ 是一个回归平面。当 $p\ge2$ 
时，无法用几何图形表示。 
...
$$y=\hat \beta_0 + \hat \beta _1 x_1 + \cdots + \hat \beta _p x_p + e$$ {#eq:model8}
...
依照式 [@eq:model11]求解 $\hat \beta _0, \hat \beta _1,\hat \beta _2,\cdots , \hat \beta _p$ 的表达式称为模型参数
...
(1) 线性性
 ...
 $\bf{\beta}$ 是随机向量 $Y$ 的一个线性变换，应用普通最小二乘法估计得到的回归系数向量 $\bf{\beta}$ 的估
计量为：

$$\hat {\bf{\beta}}  = ({X^T}X)^{ - 1}{X^T}Y$$

根据回归模型假设知，$X$ 是固定的设计矩阵，因此， $\hat {\bf{\beta}}$ 是 $Y$ 的一个线性变换。

(2) $\hat {\bf{\beta}}$ 是 ${\bf{\beta}}$ 的无偏估计。

证明：

$$E(\hat {\bf{\beta}}) = E[({X^T}X)^{ - 1}{X^T}Y]=({X^T}X)^{ - 1}{X^T}E(X{\bf {\beta}}  + {\bf{\varepsilon}})=({X^T}X)^{ - 1}({X^T}X){\bf{\beta}}={\bf{\beta}}$$
...
$$D(\hat {\bf{\beta}} ) = {\mathop{\rm cov}} (\hat {\bf{\beta}} ,\;\hat {\bf{\beta}} ) = E(\hat {\bf{\beta}}  - 
\hat {\bf{\beta}} )(\hat {\bf{\beta}}  - \hat {\bf{\beta}} )^T = \sigma ^2({X^T}X)^{ - 1}$$


证明：

$$D(\hat {\bf{\beta}} ) = {\mathop{\rm cov}} (\hat {\bf{\beta}},\;\hat {\bf{\beta}} ) = ({X^T}X)^{ - 1}X^T 
{\mathop{\rm cov}}(Y,Y)(({X^T}X)^{ - 1}X^T)^T=({X^T}X)^{ - 1}X^T \sigma ^2X({X^T}X)^{ - 1}=\sigma ^2({X^T}X)^{- 1}$$
...
=======
 $i=1,2,\cdots,n$ ，计算出 $SSR$ 和 $SSE$ ，进而得到 $F$  的值，其计算过程列在如表 \@ref(tab:tab-1) 的方差分析
表中，再由给定的显著性水平 $\alpha$ ,查 $F$ 分布表，得到临界值 $F_{\alpha} (p,n-p-1) $。
>>>>>>> 59f57f384b35ef15f12136c569dfe2cd388070cf
...
$${\hat y_i} = {\hat \beta _0} + {\hat \beta _1}{x_{i1}} + {\hat \beta _2}{x_{i2}} +  \cdots  + {\hat \beta _p}{x_{ip}}\;\;\;\;\;\;\;\;\;\;\;\;\;\;i = 1,\;2, \cdots ,n$$
...
tab2 <- read.csv('.\\result\\example data price.csv')
knitr::kable(tab2, row.names =F, align = "l", caption="影响我国1990-2014年国内生产总值的主要因素",
      longtable = TRUE, booktabs = TRUE, linesep  = "")
```


要求：以国内生产总值为因变量，固定资产投资、就业人数和社会消费品零售总额为自变量，拟合影响国内生产总值
的多元回归方程，并对回归方程进行检验，并解释模型的经济意义。

解：表 \@ref(tab:tab-2) 中的数据，国内生产总值、固定资产投资、社会消费品零售总额都是按现价核算的，需对数
...
设回归模型为：
$\mathop y\nolimits_t  = {\beta _0} + {\beta _1}{x_1} + {\beta _2}{x_2} + {\beta _3}{x_3} + \mathop \varepsilon \nolimits_t $ ，根据表 \@ref(tab:tab-3) 中的数据，本例采用SPSS19.0进行数据处理。
...
$$\mathop {\hat y}\nolimits_i  = {\rm{ - }}100548.3247{\rm{ + }}0.08{x_1}{\rm{ + }}1.8795{x_2} + 0.0557{x_3}$$
