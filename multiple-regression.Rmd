...
`r tab5[1,3]`、`r tab5[1,4]`，标准误为 `r tab5[1,5]` ，因此可以初步判断模型拟合得较好。
...
表 \@ref(tab:tab-6) 为方差分析表。从该表可知： $F=$ `r tab6[1,5]`，$P$ 值为 `r tab6[1,6]` 。 $F$ 值和 $P$  
...
间为（`r tab7[2,6]` ， `r tab7[2,7]`）；就业人数的回归系数95%的置信区间为（`r tab7[3,6]` ， `r tab7[3,7]`）；
社会消费品零售总额的回归系数95%的置信区间为（`r tab7[4,6]` , `r tab7[4,7]`）。
...
表 \@ref(tab:tab-5-1) 中的数据已剔除价格因素影响，本例可以使用excel进行数据分析步
骤如下：
...
表 \@ref(tab:tab-17) 给出了2015-2017年剔除价格因素影响的不变价数据，比较显示真实值与预测值之间存在较大偏差，
这进一步说明不满足基本假定的模型拟合效果欠佳。
